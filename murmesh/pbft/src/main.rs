    ...
    I: for<'m> State<'m, Message = client::Message>,
{
    ...
    fn update(&mut self, message: Self::Message) {
        assert_eq!(message, murmesh_pbft::client::Result(Default::default()));
        let now = Instant::now();
...
async fn run_clients(cli: Cli, route: ClientTable, replica_route: ReplicaTable) {
    ...
    for index in client_index..client_index + cli.client_count {
        ...
        let mut client = murmesh_pbft::Client::new(
            client_id,
            cli.faulty_count,
...
async fn run_replica(cli: Cli, route: ClientTable, replica_route: ReplicaTable) {
    ...
    let app = murmesh_pbft::App::new(replica_id, Null).install_filtered(
    ...
    let (mut waker, control) =
        murmesh_time::new(Closure::from(ToReplica::Timeout).install(replica_wire.state()));
