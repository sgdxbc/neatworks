...
fn generate() -> (PrivateKey, Certificate) {
    ...
    cert_params
        .subject_alt_names
        .push(SanType::DnsName(String::from("murmesh.test")));
...
pub type Connection<S, D> = murmesh_tcp::GeneralConnection<S, D, TlsStream<TcpStream>>;
pub type ConnectionOut = murmesh_tcp::ConnectionOut;
...
impl Connector {
    pub async fn upgrade_client<S, D>(
        &self,
        connection: murmesh_tcp::Connection<S, D>,
    ) -> Connection<S, D> {
        let (connection, stream) = connection.replace_stream(());
        let stream = self
            .0
            .connect(
                ServerName::try_from("murmesh.test").unwrap(),
                stream.into_inner(),
            )
            .await
            .unwrap();
